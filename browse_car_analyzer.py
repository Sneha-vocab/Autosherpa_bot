...
#     prompt = f"""You are an intelligent car sales assistant helping a customer browse the used cars. Analyze the user's message and extract relevant information.
...
#stores the analyzed data back into a json format
...
async def analyze_browse_car_message(
    ...
    *,
    ...
    """Analyze message + update stored conversation memory."""
    ...
    # Previous memory (persistent)
    data = conversation_context.setdefault("data", {})
    collected_brand = data.get("brand")
    collected_budget = data.get("budget")
    collected_type = data.get("car_type")
    ...
    - Step: {current_step}
    - Already collected: Brand={collected_brand}, Budget={collected_budget}, Type={collected_type}
    - Available brands in DB: {', '.join(available_brands)}
    - Available types in DB: {', '.join(available_types)}
    """

    prompt = f"""
    You are an intelligent used-car assistant. Analyze the user's message.
    ...
    Extract:
    - extracted_brand: exact match from {available_brands}
    - extracted_type: exact match from {available_types}
    - extracted_budget: convert lakhs → rupees
    - user_intent: e.g. providing_brand, providing_budget, providing_type,
                   asking_question, changing_criteria, selecting_car, booking_test_drive
    - needs_clarification
    ...
    Return JSON ONLY:
    {{
        "extracted_brand": "brand_or_null",
        "extracted_budget": number_or_null,
        "extracted_type": "type_or_null",
        "user_intent": "intent_string",
        "needs_clarification": true_or_false,
        "clarification_question": "question_or_null",
        "confidence": 0.0_to_1.0
    ...
    try:
        ...
        raise BrowseCarAnalysisError("Gemini API request failed") from exc

    raw = response.json()
    try:
        candidate_text = raw["candidates"][0]["content"]["parts"][0]["text"].strip()
    except Exception:
        raise BrowseCarAnalysisError("Unexpected Gemini response structure")

    try:
        parsed = json.loads(candidate_text)
    except Exception:
        raise BrowseCarAnalysisError("Failed to parse Gemini response JSON")

    budget_tuple = None
    b = parsed.get("extracted_budget_min")

    if bmin is not None or bmax is not None:
        budget_tuple = (bmin, bmax)


    changing = parsed.get("user_intent") == "changing_criteria"

    # Brand
    if parsed.get("extracted_brand"):
        if changing or data.get("brand") != parsed["extracted_brand"]:
            data["brand"] = parsed["extracted_brand"]

    # Budget
    if budget_tuple:
        if changing or data.get("budget") != budget_tuple:
            data["budget"] = budget_tuple

    # Type
    if parsed.get("extracted_type"):
        if changing or data.get("car_type") != parsed["extracted_type"]:
            data["car_type"] = parsed["extracted_type"]

    # Always track last intent
    data["last_intent"] = parsed.get("user_intent")

    # Clarification handling
    if parsed.get("needs_clarification"):
        data["pending_clarification"] = parsed.get("clarification_question")
    else:
        data["pending_clarification"] = None

    # Save updated data back
    conversation_context["data"] = data


    print("--- Browse Car Analysis Parsed Result ---")
    print(json.dumps(parsed, indent=4)) # Debug print


    return {
        "extracted_brand": parsed.get("extracted_brand"),
        "extracted_budget": budget_tuple,
        "extracted_type": parsed.get("extracted_type"),
        "user_intent": parsed.get("user_intent", "unknown"),
        "needs_clarification": parsed.get("needs_clarification", False),
        "clarification_question": parsed.get("clarification_question"),
        "confidence": parsed.get("confidence", 0.0),
        "memory": conversation_context["data"],  # ★ useful for debugging
    }
...
async def generate_browse_car_response(
    ...
    *,
    ...
    if current_step == "collecting_criteria":
        ...
        Rules:
        1. DO NOT confirm brand/budget/type. (That is handled by logic.)
        2. If all three (brand + budget + type) are available AND no confirmation is needed → 
        respond: "Great! Let me show you the best matching cars."
        3. If something is missing, ask ONLY for the missing one.
        4. Be very brief (10–15 words).
        5. Friendly, human, professional tone.

        User said: "{message}"

        Generate a brief assistant response:
        """
        ...
        system_prompt = f"""You are a friendly car sales assistant. User said: "{message}". Respond naturally and helpfully in 10-15 words."""
